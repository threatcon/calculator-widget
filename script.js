// iPhone-style calculator logic for CodePen
// Populates buttons, robust preview (no "Error" on incomplete), keyboard support
...
// - converts × ÷ − and n% -> (n/100)
// - throws on invalid final eval
...
function formatDisplayNumber(n) {
  ...
  return String(Math.round(Number(n) * 1e12) / 1e12);
}

// updateDisplay: preview but avoid showing "Error" for incomplete expressions.
...
function inputChar(ch) {
  ...
  if (ch === '.') {
    ...
    if (!m) {
      const prev = expr.slice(-1);
      if (/\d|\)/.test(prev)) expr += '0.';
      else expr += '0.';
      updateDisplay();
      return;
    }
...
function init() {
  ...
  const firstKey = grid.querySelector('.key');
  if (firstKey) firstKey.setAttribute('tabindex', '0');
}

window.__iphoneCalc = { getState: () => ({ expr, lastResult, justEvaluated }) };
